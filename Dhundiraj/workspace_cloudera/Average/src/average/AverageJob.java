...
public class AverageJob extends Configured implements Tool {
	...
	public static class AveragePartitioner extends Partitioner<Text,Text>{
		...
		public int getPartition(Text key, Text value, int numReduceTasks){
			...
			System.out.println("part_key_val: " + key + "   " + value);

			if (numReduceTasks == 1){
				return 0;
			}
			//if(numReduceTasks %2!=0){
				//return (key.toString().length() * value.toString().length())%numReduceTasks;
			//}
			char fLetter =key.toString().toLowerCase().charAt(0);
			if((int)fLetter <= ((int)'a') +12){
				System.out.println("fLetter: " + fLetter + " : sent in 0");
				return 0;
			}else{
				System.out.println("fLetter: " + fLetter + " : sent in 1");
				return 1;
			}
		}
	}
